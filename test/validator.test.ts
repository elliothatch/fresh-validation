import { Constructor, validate, validateData } from '../src';
...
class PrimitivesArray {
    ...
    flags: boolean[];
    @validate({arrayType: Number, arrayOptional:true})
    counts: number[];
...
describe('validate primitives', () => {
    test('valid input is accepted', () => {
        ...
        let primitives: Primitives = validateData(input, Primitives, 'input');
    ...
    test('invalid input is rejected', () => {
        ...
        inputs.forEach((input, index) => {
            expect(() => {
                validateData(input, Primitives, `input-${index}`);
            }).toThrow(TypeError);
        });
    });
    ...
    describe('arrays', () => {
        test('valid array input is accepted', () => {
            ...
            inputs.forEach((input, index) => {
                let primitivesArray: PrimitivesArray = validateData(input, PrimitivesArray, `input-${index}`);
    ...
    describe('objects', () => {
        test('valid nested objects', () => {
            ...
            inputs.forEach((input, index) => {
                let library: Library = validateData(input, Library, `input-${index}`);
        ...
        test('invalid nested objects', () => {
            ...
            inputs.forEach((input, index) => {
                expect(() => {
                    validateData(input, Library, `input-${index}`);
                }).toThrow(TypeError);
            });
        });
    });
...
class FruitBasket {
    @validate()
    color: string;
    @validate({union: {discriminant: 'kind', types: {
        'apple': Apple,
        'banana': Banana,
        'coconut': Coconut,
    }}})
...
describe('discriminated union', () => {
    test('valid union types are accepted', () => {
        ...
        inputs.forEach((input, index) => {
            let fruitBasket: FruitBasket = validateData(input, FruitBasket, `input-${index}`);
    ...
    test('invalid union types', () => {
        ...
        inputs.forEach((input, index) => {
            expect(() => {
                validateData(input, FruitBasket, `input-${index}`);
            }).toThrow(TypeError);
        });
    });
});
...
type PersonConstructor = Constructor<Grandparent | Parent | Child>;
...
describe('inheritance', () => {
    test('valid objects are accepted', () => {
        ...
        inputs.forEach(([input, inputConstructor], index) => {
            let validatedInput = validateData(input, inputConstructor as PersonConstructor, `input-${index}`);
    ...
    test('invalid objects', () => {
        ...
        inputs.forEach(([input, inputConstructor], index) => {
            expect(() => {
                validateData(input, inputConstructor as PersonConstructor, `input-${index}`);
            }).toThrow(TypeError);
        });
    });
});
